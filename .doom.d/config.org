...
- We will map in evil-window-map to make faster in EXWM buffers. No need to press =C-SPC o c=, which feels janky
  - So here, we prefer the Emacs style of two keypress using modifier keys vs the Vim style of three keypress w/o modifier keys
#+begin_src emacs-lisp
(after! org
...
#+RESULTS:
** Source Code: Search up frame in M-x
